...
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
//u16 USART_RX_STA=0;       //接收状态标记	//申明 “USART_RX_STA” 这个变量在其他文件中已经定义了，"u16"的意思是16位的。
...
//数据缓存区 u8 Data【100】;
//接收我全部的串口数据 -- 串口中断服务函数中进行，使用计数器累加
//编写一个读位函数--每次读数据缓存区的一位数据，开始判断，起始位，结束位，以及数据段
//将数据读取出来在进行我的数据操作
...
void USART2_IRQHandler(void)                	//串口1中断服务程序
{
	...
	u8 Res;
//	int y;
#if SYSTEM_SUPPORT_OS 		//如果SYSTEM_SUPPORT_OS为真，则需要支持OS.   //判断是否支持操作系统
	OSIntEnter();    
#endif
	if(USART_GetITStatus(USART2,USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾（下面判断）) //这是判断读数据寄存器是否空，因为接受到了字符才会产生中断，所以不是空的，这个判断成立
	{
		...
		if(Res==0x66)//开始收到一组数据
		{
			k=1;
			shoudao=0;
			y[0]=Res;
		}
		else if(Res==0x99) //结束收数据//在外面就不会多一组进行覆盖了
		{
				k=0;
				flag=1;
			 y[1]=Res;
